...
const state = {
  ...
  hasCloseRegistModal: false, // 是否手动关闭了登录注册提示
  ...
  showDownloadBar: false, // 默认不显示下载条
  hasCloseDownloadBar: false, // 是否手动关闭了下载条
  showTopicDownloadBar: true, // 默认显示石材展活动的下载条
  hasCloseTopicDownloadBar: false, // 是否手动关闭了石材展活动的下载条
  onlyMobile: false, // 点击立即购买和购物车的支付，再点登录，隐藏第三方登录按钮
  registGuidSuccess: false, // 注册成功并成为某某企业/商城的导购员/会员
  registGuidSuccessDetail: null, // 注册成功并成为某某企业/商城的导购员/会员，企业或商城的详情
  showRegistBar: false, // 顶部注册提示条
  hasCloseRegistBar: false, // 是否哦手动关闭了顶部注册提示条
  sharerInfo: null, // 分享人信息
  bindMobileModal: false, // 绑定手机号模态窗
  activityProducts: [] // 年货节活动页面产品数组
}
...
const getters = {
  ...
  showTopicDownloadBar: state => state.showTopicDownloadBar,
  hasCloseTopicDownloadBar: state => state.hasCloseTopicDownloadBar,
  onlyMobile: state => state.onlyMobile,
  registGuidSuccess: state => state.registGuidSuccess,
  registGuidSuccessDetail: state => state.registGuidSuccessDetail,
  showRegistBar: state => state.showRegistBar,
  hasCloseRegistBar: state => state.hasCloseRegistBar,
  sharerInfo: state => state.sharerInfo,
  bindMobileModal: state => state.bindMobileModal,
  activityProducts: state => state.activityProducts
}
...
const actions = {
  ...
  switchTopicDownloadBar ({commit}, params) {
    commit(types.SWITCH_TOPIC_DOWNLOAD_BAR_STATUS, {params})
  },
  closeDownloadBar ({commit}, params) {
    commit(types.CLOSE_DOWNLOAD_BAR, {params})
  },
  closeTopicDownloadBar ({commit}, params) {
    commit(types.CLOSE_TOPIC_DOWNLOAD_BAR, {params})
  },
  closeIntegralDialog ({commit}, params) {
    commit(types.CLOSE_INTEGRAL_DIALOG, {params})
  },
  switchGuidDialog ({commit}, params) {
    commit(types.SWITCH_GUID_DIALOG, {params})
  },
  setOrganizationDialogInfo ({commit}, params) {
    commit(types.ORGANIZATION_DIALOG_DETAIL, {params})
  },
  switchRegistBar ({commit}, params) {
    commit(types.SWITCH_REGIST_BAR, {params})
  },
  closeRegistBar ({commit}, params) {
    commit(types.CLOSE_REGIST_BAR, {params})
  },
  switchBindMobile ({commit}, params) {
    commit(types.SWITCH_BIND_MOBILE, {params})
  }
}
...
const mutations = {
  ...
  [types.SWITCH_INTEGRAL_DIALOG] (state, {params}) {
    ...
    state.onlyMobile = !!params.onlyMobile
  },
  ...
  [types.SWITCH_TOPIC_DOWNLOAD_BAR_STATUS] (state, {params}) {
    state.showTopicDownloadBar = params.status
  },
  ...
  [types.CLOSE_TOPIC_DOWNLOAD_BAR] (state, {params}) {
    state.hasCloseTopicDownloadBar = params.status
    state.showTopicDownloadBar = !params.status
  },
  ...
  [types.SWITCH_GUID_DIALOG] (state, {params}) {
    state.registGuidSuccess = params.status
  },

  [types.ORGANIZATION_DIALOG_DETAIL] (state, {params}) {
    state.registGuidSuccessDetail = params.detail
  },

  [types.SWITCH_REGIST_BAR] (state, {params}) {
    state.showRegistBar = params.status
  },

  [types.CLOSE_REGIST_BAR] (state, {params}) {
    state.showRegistBar = !params.status
    state.hasCloseRegistBar = params.status
  },

  [types.SWITCH_BIND_MOBILE] (state, {params}) {
    state.bindMobileModal = params.status
  }
}
