<template>
  <section>
    <div class="full-width">
      ...
      <div class="container" ref="alumniCarte" :style="{height: scrollHeight}" v-lazy:background-image="{
        src: themeBgUrl,
        ...
        <div class="qr-card-container" :class="{'only-card': serverSwitch === 'off', 'transition': hasSwitch && serverSwitch === 'on'}">
          <card-jingdianshangwu-c v-if="themeType === 'school_jingdian'" :store="teams" :sharer="sharerInfo" @qr-code="goQrCode" @icon-click="iconClick" @click="goEnterpriseDetail" @nav="navClick" @go-category="goCategory">
          </card-jingdianshangwu-c>
          <card-jianyueshangwu-c v-if="themeType === 'school_jianyue'" :store="teams" :sharer="sharerInfo" @qr-code="goQrCode" @icon-click="iconClick" @click="goEnterpriseDetail" @nav="navClick" @go-category="goCategory">
          </card-jianyueshangwu-c>
        ...
        <div class="carte-tab-container" :class="{'hide-products': serverSwitch === 'off'}">
          <template v-if="loading || products && products.length > 0">
            <div>
              ...
              <mugen-scroll :handler="loadProductBottom" :handle-on-mount="false" :should-handle="!loading" :threshold="0.1" scroll-container="alumniCarte">
                <div class="loading">
...
<script>
  ...
  import CardJianyueshangwuC from '../../components/theme/CardJianyueshangwuC'
  import CardJingdianshangwuC from '../../components/theme/CardJingdianshangwuC'
  ...
  export default {
    ...
    methods: {
      ...
      goBack () {
        if (this.hasSearch) {
          setScrollTop(0, this.$refs.alumniCarte)
          this.getProducts('', 0)
      ...
      handleSearchBtn (res) {
        ...
        setScrollTop(this.height, this.$refs.alumniCarte)
        this.searchFn(res)
      ...
      handleSearchBar () {
        if (!this.scrollActive) {
          ...
          }, this.height, this.$refs.alumniCarte, 0, 0)
        }
      },
      goScroll (scroll) {
        setScrollTop(scroll, this.$refs.alumniCarte)
      },
      ...
      goQrCode (obj) {
        ...
        this.$router.push({name: 'QrPage', query: {name: obj.company, state: obj.state, logo: obj.logo, hasHeader: true, url: `${window.location.protocol}//${window.location.host}/alumnis/${obj.id}${sharerUrl}`}})
      },
      handleScrollHeight () {
      ...
      navClick (item) {
        switch (item.index) {
          case 0:
            ...
            break
          case 1:
            this.$router.push({name: 'Organization', params: {id: item.id}, query: {title: '校友班级', type: 'class'}})
            break
          case 2:
            this.$router.push({name: 'Organization', params: {id: item.id}, query: {title: '校友企业', type: 'enterprise'}})
            break
          case 3:
            this.$router.push({name: 'Timeline', params: {id: item.id}, query: {name: this.teams.company}})
            break
          default:
            this.$router.push({name: 'Information', params: {id: item.id}})
            break
    ...
    activated () {
      ...
      if (!this.$store.state.popState || this.$store.state.fromLogin) {
        setScrollTop(parseInt(this.$route.query.height + '' || 0 + ''), this.$refs.alumniCarte)
        ...
        setScrollTop(this.$store.state.scrollMap.AlumniCarte || 0, this.$refs.alumniCarte)
      }
    },
    beforeRouteLeave (to, from, next) {
      this.$store.dispatch('saveScroll', {name: 'AlumniCarte', value: this.$refs.alumniCarte.scrollTop})
      if (to.name !== 'ProductDetail' && to.name !== 'InformationFolders' && to.name !== 'Chat' && to.name !== 'Login' && to.name !== 'BeforeRegister' && to.name !== 'Help' && to.name !== 'Maps' && to.name !== 'ShoppingCart' && to.name !== 'EnterpriseDetail' && to.name !== 'Report' && to.name !== 'Download' && to.name !== 'QrPage' && to.name !== 'Information' && to.name !== 'Organization' && to.name !== 'Timeline') {
        this.showGoTopBtn = false
        this.showSearchBar = false
        this.productPageIndex = 1
