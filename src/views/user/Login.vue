  ...
  export default {
    ...
    data () {
      return {
        ...
        qqLogin: `${AUTH_URL}/member/auth/qq_connect?url=${encodeURIComponent('/login?provider=qq_connect&tmp_token=')}`,
    ...
    methods: {
      goBack (social = false, provider = '') {
        if (getStore('fromShoppingCart')) {
          ...
          this.$router.go(social ? (provider === 'qq_connect' ? -3 : -2) : -1) // beforeLogin优先级较高
        } else if (getStore('Login_goHome')) {
          removeStore('Login_goHome')
          this.$router.replace({name: 'See'})
        } else {
          this.$router.go(social ? (provider === 'qq_connect' ? -3 : -2) : -1)
        }
      },
      login () {
      ...
      shouldLogin () {
        ...
        } else if (getStore('weixinLogin')) {
          /* Toast({
            message: '自动登录失败，请手动登录',
            duration: 1000
          }) */
          return false
        } else if (mobileClient() === 'weixin' && !getStore('weixinLogin')) {
