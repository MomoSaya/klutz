  ...
  <section class="container">
    ...
    <template v-if="purchaseItems && purchaseItems.length > 0">
      ...
      <div class="option-bar full-width">
        <a
          class="flex-between check-all-box"
          @click="handleAllCheck">
          <i
            v-if="checkAll"
            class="iconfont icon-xuanzhong primary"></i>
          <i
            v-if="!checkAll"
            class="iconfont icon-weixuanzhong third-text"></i>
  ...
  export default {
    ...
    methods: {
      ...
      handleTeams (teams, files) {
        ...
        for (let i = 0; i < teams.length; i++) {
          for (let j = 0; j < teams[i].purchase_items.length; j++) {
            ...
            teams[i].purchase_items[j].price.money = teams[i].purchase_items[j].price.promotion_price || teams[i].purchase_items[j].price.money
          }
        }
        return tmpArr
      },
      ...
      handleTotalMoney () {
        ...
        for (let i = 0; i < this.purchaseItems.length; i++) {
          for (let j = 0; j < this.purchaseItems[i].purchase_items.length; j++) {
            if (this.purchaseItems[i].purchase_items[j].checked && !isNaN(parseFloat(this.purchaseItems[i].purchase_items[j].price.promotion_price || this.purchaseItems[i].purchase_items[j].price.money + ''))) {
              totalMoney += parseFloat(this.purchaseItems[i].purchase_items[j].price.promotion_price || this.purchaseItems[i].purchase_items[j].price.money + '') * parseInt(this.purchaseItems[i].purchase_items[j].quantity + '')
            }
          }
        }
