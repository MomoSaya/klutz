<template>
  <section class="full-width">
    ...
    <section  v-if="!loading && menuList.length === 0" class="empty-tips">
      <p class="font-20 third-text">该机构暂无产品分类</p>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goSingleCategory (id) {
        this.$router.push({name: 'CategoryProducts', params: {id: id}, query: {teamId: parseInt(this.id + '')}})
      },
      getCategories () {
        this.$store.dispatch('commonAction', {
          url: '/v1/categories/navigation',
          method: 'get',
          params: {
            ...((this.id + '' !== '1') ? {organization_id: parseInt(this.id + '')} : {})
          },
          target: this,
          resolve: (state, res) => {
      ...
      searchCategories (queryParams) {
        this.$store.dispatch('commonAction', {
          ...
          method: 'get',
          params: {
            ...((this.id + '' !== '1') ? {team_id: parseInt(this.id + '')} : {}),
            q: queryParams
          },
          target: this,
          resolve: (state, res) => {
        ...
        this.menuScroll.on('scroll', (pos) => {
          ...
          this.productListTop.forEach((item, index) => {
            if (this.menuIndexChange && Math.abs(Math.round(pos.y)) >= item) {
              ...
              let el = menuList[0]
              // 左侧菜单栏每栏高度为96px， 除以2为菜单栏数量较多时，激活的菜单居中显示。
              wrapperMenu.scrollToElement(el, 800, 0, -(wrapMenuHeight - 96) / 2)
            }
          })
        })
      },
      ...
      chooseMenu (index) {
        ...
        this.menuScroll.scrollTo(0, -this.productListTop[index], 400)
