<template>
  <section>
    ...
    <div
      v-if="isWechat"
      class="options-text">
      <p class="tips-title">是否已完成付款?</p>
      <p class="tips-content">
        <span>请确保支付完成后，点击</span>
        <span class="btn" @click="goBack()">完成付款</span>
      </p>
    </div>
  </section>
</template>
    ...
    methods: {
      init () {
        ...
        if (ua.indexOf('micromessenger') !== -1) {
          ...
          let param = getQueryString(location.href, 'goto') || ''
          location.href = param !== '' ? _AP.decode(param) : `payerror/${this.$route.query.code}?amount=${this.$route.query.amount || ''}`
        }
      },
      goBack () {
        if (getStore('PayTips_goHome')) {
          removeStore('PayTips_goHome')
          this.$router.push({name: 'See'})
        } else {
          this.$router.replace({name: 'Pay', query: {back: '1', code: this.$route.query.code || '', amount: this.$route.query.amount || ''}})
        }
      },
    },
    mounted () {
...
<style lang="scss" scoped>
  ...
  .weixin-tip {
    ...
    @include pm2rem(padding, 110px, 0px, 0px, 0px);
    text-align: left;
  ...
  .weixin-tip-img {
    display: none;
    @include pm2rem(padding, 220px, 0px, 0px, 0px);
  }
