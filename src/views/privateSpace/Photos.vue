...
<script>
  ...
  export default {
    ...
    methods: {
      beforeGetData () {
        if (this.p) {
          this.getData('/v1/shares/photos', {p: this.p, id: this.id, page: this.pageIndex, per_page: this.pageSize})
        } else {
          this.shouldLogin()
        }
      },
      ...
      shouldLogin () {
        if (!this.token) {
          ...
          this.getData(`/v1/galleries/${this.id}/photos`, {token: this.token, page: this.pageIndex, per_page: this.pageSize})
        }
      }
    },
    mounted () {
